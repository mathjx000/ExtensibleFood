...
public final class FoodInfoCommand {
	...
	public static void register(final CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(CommandManager.literal("foodinfo").then(CommandManager.argument("item", new FoodStackArgumentType()).executes(ctx -> {
			...
			lines.add(new TranslatableText("commands.extensible_food.foodinfo.entry_saturation", createNumberText(Utils.saturationRatioToHumanReadableSaturationPoints(component.getHunger(), component.getSaturationModifier())), createNumberText(component.getSaturationModifier())));
