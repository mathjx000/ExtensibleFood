...
public final class FoodTooltip {
	...
	public static void append(ItemStack stack, TooltipContext context, List<Text> list) {
		...
		if (item.isFood()) {
			...
			list.add(new TranslatableText("tooltip.extensible_food.saturation", formatFloatingValue((double) food.getSaturationModifier())).formatted(Formatting.DARK_GREEN));
	...
	public static Text formatFloatingValue(double value) {
		if (value < 0D) {
			return new TranslatableText("tooltip.extensible_food.value.negative", ItemStack.MODIFIER_FORMAT.format((double) -value));
		} else return new TranslatableText("tooltip.extensible_food.value.positive", ItemStack.MODIFIER_FORMAT.format((double) value));
	}
